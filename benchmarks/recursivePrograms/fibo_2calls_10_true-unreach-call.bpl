...
function {:existential true} {:inline} Ens1(n: int, result: int, result_aux: int) : bool;
//function {:existential true} {:inline} Req2(n: int) : bool;
function {:existential true} {:inline} Ens2(n: int, result: int, result_aux: int) : bool;

procedure fibo1(n: int) returns (result: int)
//requires Req1(n);
ensures Ens1(n, result, result + 1);
{
	var fibo_n_1: int;
	var fibo_n_2: int;
    	if (n < 1) {
...
//requires Req2(n);
ensures Ens2(n, result, result + 1);
{
	var fibo_n_1: int;
	var fibo_n_2: int;
    	if (n < 1) {
